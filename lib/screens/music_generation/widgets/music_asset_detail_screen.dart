...
import 'music_detail_dialog.dart';

class MusicAssetDetailScreen extends StatefulWidget {
  final MusicAsset asset;

  const MusicAssetDetailScreen({
    Key? key,
    required this.asset,
  }) : super(key: key);

  @override
  State<MusicAssetDetailScreen> createState() => _MusicAssetDetailScreenState();
}

class _MusicAssetDetailScreenState extends State<MusicAssetDetailScreen> {
  late MusicAsset _currentAsset;
    ...
    return Center(
      child: Column(
        children: [
          ...
          const SizedBox(height: 8),
          const Text(
            'Start generating music for this asset',
            style: TextStyle(
  ...
  Widget _buildDesktopGrid(List<MusicGeneration> generations) {
  ...
  Widget _buildMobileList(List<MusicGeneration> generations) {
  ...
  Widget _buildGenerationCard(MusicGeneration generation) {
    return Card(
      color: const Color(0xFF2A2A2A),
  ...
  Widget _buildGenerationListTile(MusicGeneration generation) {
    return Card(
      color: const Color(0xFF2A2A2A),
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(8),
  ...
  void _toggleFavorite(MusicGeneration generation) async {
    try {
  ...
  void _showGenerationDetails(MusicGeneration generation) async {
    await showDialog(
      context: context,
